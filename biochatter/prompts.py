...
class BioCypherPromptEngineV2(BioCypherPromptEngine):
    def __init__(
        self,
    ...
    def _generate_query(
        ...
        msg = (
            f"Generate a database query in {query_language} that answers "
            f"the user's question. "
            f"You can use the following DATABASE YAML schema to construct the {query_language} query."
            f"Write all entities in PascalCase."
            f"Only return the query, without any additional text. Do not prepend ```cypher to the query."
            f"DATABASE YAML: {schema}"
        )
